function getBotResponse() {
  ...
  //   var user_message_html =
  //     '<p class="user_message"><span>' + user_input_text + "</span></p>";

  //   $("#chat_holder").append(user_message_html);
  $.get("/get", { user_query: $.trim(user_input_text) }).done(function (
    bot_response_list
  ) {
    $.each(bot_response_list, function (index, response) {
      ...
      if (response.trim()) {
        var bot_response_html =
          '<p class="bot_message"><span>' + response.replace("\n", "</br>");
        +"</span></p>";

        console.log(bot_response_html);
        console.log(response);
        // $("#chat_holder").append(bot_response_html);
        // $("#chat_holder").animate(
        //   { scrollTop: $("#chat_holder").prop("scrollHeight") },
        //   1000
        // );
      }
    });
